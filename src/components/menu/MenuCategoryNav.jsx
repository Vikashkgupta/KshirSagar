...
const REQUESTED_CATS = [
  ...
  { name: 'Snacks', filter: 'Snacks', img: '/snak.jpg', emoji: '🍟' },
  { name: 'Sweets', filter: 'Sweets', img: '/sweet.jpg', emoji: '🍬' },
  ...
  { name: 'Namkeen', filter: 'Namkeen', img: '/namkeen.jpg', emoji: '🥨' }
];

const MenuCategoryNav = ({ activeCategory, setActiveCategory }) => {
  return (
    <>
      ...
      <div className="px-[clamp(1rem,4vw,2.5rem)] pb-[clamp(1rem,3vw,1.5rem)] flex flex-col items-start bg-white !important">
        <div className="inline-flex flex-col items-start gap-[0.4rem]">
          <div className="font-playfair text-[clamp(1.4rem,5vw,2rem)] font-bold text-[#111] flex items-center gap-[0.6rem]">
            {REQUESTED_CATS.find(c => c.filter === activeCategory)?.emoji}{' '}
            {REQUESTED_CATS.find(c => c.filter === activeCategory)?.name}
          </div>
